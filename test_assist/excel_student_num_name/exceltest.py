...
def test_name_get():
    ...
    m3 = random.randint(0, len(ming) - 1)
    test_sead = random.randint(2, 4)
    if test_sead == 2:
        test_name = xing[x] + ming[m1]
    if test_sead == 3:
        test_name = xing[x] + ming[m1] + ming[m2]
    if test_sead == 4:
        test_name = xing[x] + ming[m1] + ming[m2] + ming[m3]
    return test_name

def excel_out():
    workbook = xlsxwriter.Workbook('test.xls')
    worksheet = workbook.add_worksheet()
    worksheet.write('A1', '考试年份')
    worksheet.write('A2', '考号')
    worksheet.write('B1', '2018')
    worksheet.write('B2', '姓名')
    for i in range(1, 30000):
        id = str(("%010d" % (i)))
        worksheet.write('A' + str(i + 2), id)
        worksheet.write('B' + str(i + 2), test_name_get())
    workbook.close()
